trigger AccountTrigger on Account( before insert, after insert,
  ...
  if (Trigger.isAfter && Trigger.isInsert) {
    ...
    // We are about to insert contacts for each Account entered the
    //trigger, so we need a List<Contact> to store it and eventually
    ...
    for(Account each : Trigger.new) {
      ...
      if(each.AnnualRevenue > 1000000){
        ...
        // Create a Sample Contact for (under) this Account
    ...
    // You have prepared your list, now it`s time to insert; (DML)
    insert contactList;
  }
